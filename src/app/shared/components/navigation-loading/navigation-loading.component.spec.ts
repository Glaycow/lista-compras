...
function emitRouterEvent(router: Router, event: Event): void {
...
describe('NavigationLoadingComponent', () => {
  ...
  it('should show loading bar on NavigationStart', () => {
    emitRouterEvent(router, new NavigationStart(1, '/test'));
    fixture.detectChanges();
  ...
  it('should hide loading bar on NavigationEnd after delay', () => {
    ...
    emitRouterEvent(router, new NavigationEnd(1, '/test', '/test'));
  ...
  it('should hide loading bar on NavigationCancel after delay', () => {
    ...
    emitRouterEvent(router, new NavigationCancel(1, '/test', 'User cancelled'));
    expect(component['loading']()).toBe(true); // Still true during the 150ms delay
  ...
  it('should hide loading bar on NavigationError after delay', () => {
    ...
    emitRouterEvent(router, new NavigationError(1, '/test', new Error('Failed')));
    expect(component['loading']()).toBe(true); // Still true during the 150ms delay
  ...
  it('should clear timeout on consecutive navigations', () => {
    vi.useFakeTimers();
    emitRouterEvent(router, new NavigationStart(1, '/first'));
    expect(component['loading']()).toBe(true);

    // End first navigation (starts 150ms timer)
    emitRouterEvent(router, new NavigationEnd(1, '/first', '/first'));

    // Start second navigation immediately (should clear the first timer)
    emitRouterEvent(router, new NavigationStart(2, '/second'));
    expect(component['loading']()).toBe(true);
    ...
    emitRouterEvent(router, new NavigationEnd(2, '/second', '/second'));
    vi.advanceTimersByTime(150);
    fixture.detectChanges();
